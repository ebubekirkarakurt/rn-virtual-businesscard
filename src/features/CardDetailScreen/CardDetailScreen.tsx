...
const CardDetailScreen = ({route}: Props) => {
  ...
  const makeCall = (phoneNumber: string) => {
    Linking.openURL(`tel:${phoneNumber}`).catch((err) => Alert.alert("Error", "Cannot make a call"));
  };
  
  const sendEmail = (email: string) => {
    Linking.openURL(`mailto:${email}?`).catch((err) => Alert.alert("Error", "E-mail cannot send"));
  };
    ...
    <View style={styles.main}>
      ...
      <View style={{ alignItems:'center', justifyContent:'center',}} >
        ...
        <View style={styles.row}>
          <ConnectMe iconName={'instagram'} onPress={() => openLink(item.instagram)} />
          <ConnectMe iconName={'linkedin'} onPress={() => openLink(item.linkedin)} />
        </View>
      </View>
    </View>
  )
